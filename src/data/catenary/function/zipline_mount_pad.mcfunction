...
import numpy as np
from eroxified2:custom_block import CustomBlock, PlacedBlockStonePressurePlate, BlockComponents
from catenary:config import Config

class ZiplineMountPad(CatenaryItem):
  base_item = "comparator"
...
class ZiplineMountPad(CustomBlock):
  placed_block = PlacedBlockStonePressurePlate
  item = ZiplineMountPad
  entity_tags = ["catenary.entity", "catenary.custom_block"]
  ...
  def on_placed(cls):
    ...
    execute align y run summon item_display ~ ~ ~ {Tags:["catenary.entity",f"catenary.custom_block.id.{cls.id}.skin"],item:{id:"minecraft:player_head",components:{"minecraft:profile":skin}},transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,0.07f,0f],scale:[1f,0.125f,1f]}}
    
  class powered_check(BlockComponents.PoweredStateCheck):
    def on_powered(cls):
  ...
  def on_broken(cls):
    execute align y run kill @e[type=item_display,tag=f"catenary.custom_block.id.{cls.id}.skin",distance=..0.1]
