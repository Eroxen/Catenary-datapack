...
execute on target if items entity @s weapon.mainhand minecraft:firework_rocket[minecraft:custom_data~{catenary:{detect:true}}] run return run function catenary:rocket/api/click_anchor

data remove storage catenary:calc anchor.interaction
execute on target if items entity @s weapon.mainhand * run function catenary:anchor/internal/interaction/get_interaction with entity @s SelectedItem
