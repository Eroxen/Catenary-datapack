...
data modify storage catenary:calc zipline.data set value {rope_pos_i:0,rope_pos_f:0f,speed:0.15f,name:"Zipline"}
execute if score #zipline.spawn_with.speed catenary.calc matches 1.. store result storage catenary:calc zipline.data.speed float 0.001 run scoreboard players get #zipline.spawn_with.speed catenary.calc
execute if score #zipline.spawn_with.speed catenary.calc matches 1.. run scoreboard players reset #zipline.spawn_with.speed catenary.calc
...
execute if data storage catenary:calc zipline.spawn.trajectory{direction:"down"} store result storage catenary:calc zipline.data.speed float 0.001 run data get storage catenary:calc zipline.data.speed -1000
...
scoreboard players operation @s catenary.id = #catenary_id catenary.calc
